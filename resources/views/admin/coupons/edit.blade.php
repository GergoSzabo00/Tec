    ...
    <div class="row g-3">
        <div class="col-12 col-md-9 col-lg-6">
            <div class="card">
                <div class="card-body">
                    <div class="row g-3">
                        <x-forms.input id="code" name="code" icon="rectangle-list" label="{{ __('Code') }}" placeholder="{{ __('Code') }}" value="{{ $coupon->code }}" />
                        <div>
                        ...
                        <div>
                            ...
                            <select id="typeSelect" name="type" class="form-select form-select-lg rounded-pill mb-2">
                                <option {{ ($coupon->type == 'numeric') ? 'selected' : '' }} value="numeric">{{ __('Numeric') }}</option>
                                <option {{ ($coupon->type == 'percentage') ? 'selected' : '' }} value="percentage">{{ __('Percentage') }}</option>
                            </select>
                        </div>
                        <x-forms.input type="number" id="coupon_value" name="coupon_value" icon="dollar" label="{{ __('Value') }}" placeholder="{{ __('Value') }}" value="{{ $coupon->coupon_value }}" min=".01" max="99999999.99" step=".01" />
                        <x-forms.input type="number" id="minimum_cart_amount" name="minimum_cart_amount" icon="dollar" label="{{ __('Minimum cart amount') }}" placeholder="{{ __('Minimum cart amount') }}" value="{{ $coupon->minimum_cart_amount }}" min="0" max="99999999.99" step=".01" />
                        <x-forms.input type="date" id="expires_at" name="expires_at" label="{{ __('Expires at') }}" placeholder="{{ __('Expires at') }}" value="{{ $coupon->expires_at }}" />
                    </div>
                </div>
            </div>
        </div>
